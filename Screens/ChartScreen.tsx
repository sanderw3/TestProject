        ...
        Object.keys(students).forEach((studentName) => {
            ...
            if (grade != null){ 
                switch(grade){ 
                    ...
                    case "D": returningData.datasets[0].data[3]++; break;
                    case "E": returningData.datasets[0].data[4]++; break;
                    case "F": returningData.datasets[0].data[5]++; break;
                }
            }
        });
    } catch(error){
        ...
        <View style={styles.container}>
            <ScrollView  style={{width: "100%"}} contentContainerStyle={styles.scrollView}>
                {Object.keys(users).map((className) => (
                    users[className]["students"] ? //checcks if any students are actually assigned to the class:
                    <GradeChart key={className} class={users[className]} classId={className} />
                    : null
                ))}
